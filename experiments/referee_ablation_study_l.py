...
if p < 2:
    alpha = (p - 1) / 4
    r = 1.
    y = np.random.random((REFEREE_DIMENSION, 1))
    ...
    r = 2 / p
    y = np.random.random((REFEREE_DIMENSION, 1))
    y = (lmbda + REFEREE_DIMENSION ** (1 / q - 1 / p)) * y / lpnorm(y, q)
    L = REFEREE_DIMENSION ** (1 / 2 - 1 / p)
    M = L * (alpha * lmbda) ** (-2 / p)

M_0 = 4 * L
M = max(M, M_0)

A = np.identity(REFEREE_DIMENSION)

objective_function = SquaredLoss(A=A, b=y)
feasible_region = LpBall(dimension=REFEREE_DIMENSION, p=p)
...
file_name = ("referee_ablation" + "_p=" + str(round(p, 2)))

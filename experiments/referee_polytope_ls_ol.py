...
z = np.random.random((REFEREE_DIMENSION, 1))
z[0] = 0
z[1] = 0
z = rho * z / lpnorm(z, 1)
z[1] = (1 - rho)
offset = kappa * np.ones((REFEREE_DIMENSION, 1))
offset[0] = 0
y = z + offset
A = np.identity(REFEREE_DIMENSION)
objective_function = SquaredLoss(A=A, b=y)
feasible_region = LpBall(dimension=REFEREE_DIMENSION, p=1)
...
all_primal_dual_gaps, _, _, _, _ = create_reference_lines_automatically(all_primal_dual_gaps, labels, None, None, gap_0,
                                                                        iterations=ITERATIONS_MANY)
file_name = ("referee_polytope_ls_ol" + "_l1_ball" + "_rho=" + str(rho) + "_kappa=" + str(kappa))
